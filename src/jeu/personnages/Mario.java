...
public class Mario extends Personnage {
    ...
    public Image saute(){		
		...
		if(this.compteurSaut <= 40){
			if(this.getY() > JeuMario.scene.getHautPlafond()){this.setY(this.getY() - 4);}
			else{this.compteurSaut = 41;}			
			if(this.isVersDroite() == true){str = "/images/marioSautDroite.png";}
			else{str = "/images/marioSautGauche.png";}			
		// Retombé du saut
		}else if(this.getY() + this.getHauteur() < JeuMario.scene.getySol()){this.setY(this.getY() + 1);
			if(this.isVersDroite() == true){str = "/images/marioSautDroite.png";}
			else{str = "/images/marioSautGauche.png";}		
		// Saut termin�	
		}else{				
	...
	public Image marche(String nom, int frequence) {
		...
		if (this.marche == false || JeuMario.scene.getxPos() <= 0 || JeuMario.scene.getxPos() > 4430) {
	...
	public void contact(Objet objet) {		
        ...
        if(super.contactDessous(objet) == true && this.saut == true){
			JeuMario.scene.setySol(objet.getY());			
		}else if(super.contactDessous(objet) == false){
			JeuMario.scene.setySol(293); // altitude du sol initial
			if(this.saut == false){this.setY(243);}
		}
        if(super.contactDessus(objet) == true){
			JeuMario.scene.setHautPlafond(objet.getY() + objet.getHauteur()); // le plafond devient le dessous de l'objet
		}else if(super.contactDessus(objet) == false && this.saut == false){
			JeuMario.scene.setHautPlafond(0);
		}      
	}
