...
import jeu.affichage.CompteARebours;
import jeu.affichage.Score;
import jeu.audio.Audio;
import jeu.objets.Bloc;
import jeu.objets.Objet;
import jeu.objets.Piece;
import jeu.objets.TuyauRouge;
import jeu.personnages.Champ;
import jeu.personnages.Mario;
import jeu.personnages.Tortue;
...
public class Scene extends JPanel {
	...
	private int dx; // deplacement horizontal du fond d'ecran.
	...
	public Scene(){		
		...
		this.xFond1 = -50; // L'image de fond dépasse des 2 cotés de la fenétre.
		this.xFond2 = 750;
		this.ok = true;
		...
		Thread chronoEcran = new Thread(new Chrono()); // Ajout d'un chronométre à PanJeu
		chronoEcran.start(); // Démarrage du chronométre (appel de la méthode run() de la classe Chrono)
	}
	...
	public void deplacementFond() { // Déplacement du fond lorsque mario se déplace
		...
		// Remise é zéro des abscisses pour rotation des images de fond
	...
	public void paintComponent(Graphics g) {
    	...
    	// Détection des contacts de mario avec des pi�ces
 		...
 		// Image de mario
 		if(JeuMario.scene.mario.isVivant() == true){
